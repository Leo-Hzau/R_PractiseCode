...
#因为dplyr包用C语言开发，对处理tbl对象是非常迅速的。语法：
tbl_df(src, ...)
...
#变量选取
#filter()函数只能将指定条件的观测筛选出来，并不能筛选出只关心的变量，为了弥补这个缺陷，
#可以使用select()函数筛选指定的变量，而且比subset()函数更灵活，而且选择变量的同时也可以重新命名变量。
#如果剔除某些变量的话，只需在变量前加上负号“-”。
#之所以说他比subset()函数灵活，是因为可以在select()函数传递如下参数：
...
#数据扩展
#通过mutate()函数可以在原始数据集的基础上扩展新变量，类似于transform()函数，语法如下：
mutate(.data, ...)
#例子：
